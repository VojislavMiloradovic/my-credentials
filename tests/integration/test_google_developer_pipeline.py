...
class TestGoogleDeveloperParsers:
    ...
    def test_parse_local_learnings_txt(self, temp_dir, sample_google_developer_learnings):
        ...
        with patch("update_google_developer.LEARNINGS_TXT_PATH", str(learnings_file)):
            ...
            assert learnings[0]["title"] == "Setup Basic OpenTelemetry Plugin in gRPC Python"
    ...
    def test_parse_local_learnings_txt_missing_file(self, temp_dir):
        with patch("update_google_developer.LEARNINGS_TXT_PATH", str(temp_dir / "nonexistent.txt")):
            learnings = parse_local_learnings_txt()
            assert learnings == []
...
class TestGoogleDeveloperPipelineIntegration:
    ...
    def test_main_pipeline_success(self, temp_dir, sample_google_developer_rpc, sample_google_developer_learnings, mock_archiver, mock_loss_guard, mock_retired_rules):
        learnings_file = temp_dir / "google_learnings.txt"
        learnings_file.write_text(sample_google_developer_learnings)
